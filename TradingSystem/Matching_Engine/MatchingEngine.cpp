...
std::optional<Trade> MatchingEngine::matchLimitOrders(){ 
    ...
    try {
        ...
        if(bidPrice >= askPrice){
            ...
            // std::cout << "matchLimitOrders: after reduce - bidVol=" << bestBid->GetVolume() << " askVol=" << bestAsk->GetVolume() << std::endl;

            Trade trade(bestBid, bestAsk, tradeVolume, tradePrice);
            
            if(bestAsk->GetVolume() == 0){
                // std::cout << "matchLimitOrders: removing ask " << bestAsk->getOrderId() << std::endl;
                orderBook.removeAsk(bestAsk->getOrderId());
            }
            if(bestBid->GetVolume() == 0){
                // std::cout << "matchLimitOrders: removing bid " << bestBid->getOrderId() << std::endl;
                orderBook.removeBid(bestBid->getOrderId());
            }
            return trade;
        } else {
            // std::cout << "matchLimitOrders: no match, spread=" << (askPrice - bidPrice) << std::endl;
        }
